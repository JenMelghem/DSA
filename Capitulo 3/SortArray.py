...
class Array(object):
   ...
   #Metedono mediana() 3.2
   def mediana(self):
      datos_ordenados = sorted (self.__a[:self.nElementos])
      test='sorted (self.__a[:self.nElementos])'#añadido con el ing
      elapsed = timeit.timeit(test, number=100, globals=globals())
      print(test, "Su tiempo fue:", elapsed, "seconds", flush=True)

      return median (datos_ordenados) #uso de libre
