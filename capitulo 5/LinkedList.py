...
class Link(object):            # One datum in a linked list
   def __init__(self, datum, next=None): # Constructor
      ...
      self.__next = None       # Reference to next Link
...
class LinkedList(object):      # A linked list of data elements
   ...
   def getLast(self): return self.__last # Return the first link
   ...
   def setFirst(self, link):   # Change the first link to a new Link
      ...
      # #Haciendo que sea circular
      #    if not self.isEmpty():
      #       last_link = self.getLast()
      #       last_link.setNext(self.__first)
      ...
      if self.Last() is None:
         ...
         #link.setNext.__next = self.__last #enlazando
   ...
   def getLast(self):
      ...
      link = self.getFirst()
      print ("//"*30)
      print (link.getNext(), "---", self.getFirst())
      print ("//"*30)
      while link.getNext() is not self.getFirst():  # Recorrer la lista hasta el último Link
         link = link.getNext()
      return link
   ...
   def __str__(self):          # Build a string representation
      ...
      link = self.getFirst()   # Start with first link
      #while link is not None:  # Keep going until no more links
      while link != self.getLast:  ##cambiarlo a un do while para que no se me pierda don
