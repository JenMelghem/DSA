...
class Stack(object):
   ...
   def push(self, item):               # Insert item at top of stack
      #4.1
      if self.isFull():
         raise Exception("La pila está llena, no se pueden agregar mas elementos")
   ...
   def pop(self):                      # Remove top item from stack
     #4.1
      if self.isEmpty():
            raise Exception("La pila ya está vacía, no hay nada que eliminar")
   ...
   def peek(self):                     # Return top item
    ...
    # 4.1
    if self.isEmpty():
       raise Exception("La pila está vacia, no hay nada que mostrar")
   
    self.__stackList[self.__top] # Return the top item
   ...
   def es_palindroma(self, palabra):
